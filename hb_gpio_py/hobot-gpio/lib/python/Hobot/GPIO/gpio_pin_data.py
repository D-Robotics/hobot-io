...
def get_all_pin_data():
    ...
    for n, x in enumerate(pin_data):
        if x[0] is None:
            continue
        gpio_chip_name = x[0]
        gpio_chip_dir = SYSFS_PLATFORM_PATH + gpio_chip_name #/sys/devices/platform/soc/34000000.a55_apb0/34120000.gpio
    ...
    for n, x in enumerate(pin_data):
        if x[6] is None:
            continue
        pwm_chip_name = x[6]
        pwm_chip_dir = SYSFS_PLATFORM_PATH + pwm_chip_name #/sys/devices/platform/soc/34000000.a55_apb0/34170000.pwm
